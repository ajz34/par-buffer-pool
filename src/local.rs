...
//! - there is no `drain`: another thread's slot is unreachable by design
//!   (that is the lock-free lease), so reclamation is the drop sweep and
//!   thread exit below — use [`BufferPool`](crate::BufferPool) when the
//!   whole pile must be handed back mid-life,
